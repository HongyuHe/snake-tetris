...
class GameController(val randomGen: RandomGenerator,
  ...
  val nextTetroQueue: mutable.Queue[TetrisBlock] = new mutable.Queue[TetrisBlock]()
  initQueue

  def invokeTetrominoAction(action: TetrominoAction): Unit = {
    saveCurrTetromino()
    action match {
      case Hold         =>  holdAndSpawnTetro()
  ...
  def initQueue: Unit =  for (_ <- Range(0, 3)) nextTetroQueue.enqueue(BlockTypes(randomGen.randomInt(BlockTypes.length)))  // fill queue

  def getNextTetroQueue: Vector[TetrisBlock] = nextTetroQueue.toVector
  def getHoldedTetromino: TetrisBlock = holdTetromino.blockType

  def holdAndSpawnTetro(): Unit = {
    if (!holdingFlag) {
      holdingFlag = true  // toggle flag
      holdTetromino = Tetromino makeDeepCopy currTetromino
  ...
  private[this] var holdTetromino = Tetromino()
  ...
  private[this] def spawnTetromino: Tetromino = {
    val newTetro = Tetromino(spawnTetroCenter, nextTetroQueue.dequeue())
    nextTetroQueue.enqueue(BlockTypes(randomGen.randomInt(BlockTypes.length)))  // add new tetromino type
//    holdingFlag = false
    newTetro
  }
