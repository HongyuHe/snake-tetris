...
class SnakeGame extends GameBase { // GameBase -> PApplet

  var gameLogic = new SnakeLogic()
  val updateTimer = new UpdateTimer(FramesPerSecond) // game clock
  ...
  override def draw(): Unit = {
    // This is the main code block which will be executed by Processing on each update.
    updateState()
    drawGrid()
  ...
  def drawGrid(): Unit = {
    ...
    for (y <- 0 until gameLogic.nrRows;
      ...
      // Use `getGridTypeAt()` to check the type of each cell in `nrRows` by `nrColums` matrix and
      // draw correspondent graphics based on the cells' type.
