...
case class SnakeLogicWrapper(logic : SnakeLogic) extends GameLogicInterface[SnakeAction, SnakeGridTypeWrapper]{
  ...
  def getGridTypeAt(col: Int , row: Int): SnakeGridTypeWrapper = SnakeGridTypeWrapper(logic.getGridTypeAt(col,row))
...
object SnakeRecord extends
  GenericRecord
    [SnakeAction, SnakeGridTypeWrapper, SnakeLogicWrapper, (Int,Int)](){


  override def makeGame(r: RandomGenerator, info: (Int, Int)): SnakeLogicWrapper = new SnakeLogicWrapper(new SnakeLogic(r,info._1, info._2))

  override def gameLogicName: String = "SnakeLogic"

  def charToGridType(char: Char) : SnakeGridTypeWrapper = SnakeGridTypeWrapper(char match {
    case '.' => Empty()
    case '<' => SnakeHead(West())
    case '>' => SnakeHead(East())
    case '^' => SnakeHead(North())
